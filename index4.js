...
app.get('/', (req, res) =>{ 
	...
	res.send('--------------');
	res.send('check out: displayData2/')
...
app.post('/api/heartbeat', (req, res) => {
	...
	//real time data
	var today = new Date();
	io.sockets.emit('temp', {date: today.getDate()+"-"+today.getMonth()+1+"-"+today.getFullYear(), time: (today.getHours())+":"+(today.getMinutes()), temp:data['heat']});
});
...
icanhazip.IPv4().then(function(myIP) {
	...
	console.log('tempChart: static temperature line chart chart')
	console.log('------------------')
...
//try to make real time stuff here
// view engine setup
app.set('views', 'views');
...
//var today = new Date();
//io.sockets.emit('temp', {date: today.getDate()+"-"+today.getMonth()+1+"-"+today.getFullYear(), time: (today.getHours())+":"+(today.getMinutes()), temp:10});
